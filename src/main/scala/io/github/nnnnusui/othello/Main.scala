...
object Main extends App:
  ...
  @tailrec
  def fromStdIn(actions: Seq[Action]): Action =
...
/*
 * build othello game (Othello(...))
 * loop:
 *   subscribe game state (game.board)
 *   subscribe actions (game.moves.keys)
 *   post action (=> game.moves.get(action))
 */
